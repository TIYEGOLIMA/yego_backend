...
@RestController
@RequestMapping(value = "/api/yego-gantt/tasks", produces = MediaType.APPLICATION_JSON_VALUE + ";charset=UTF-8")
...
public class AreaTaskController {
    ...
    public ResponseEntity<List<AreaTaskResponseDto>> list(Authentication authentication,
        ...
        return ResponseEntity.ok(areaTaskService.list(userId, areaId, priority));
    }
    ...
    public ResponseEntity<AreaTaskKpisResponseDto> kpis(Authentication authentication,
                                                        ...
                                                        @RequestParam(required = false) AreaTaskPriority priority) {
        Long userId = Long.parseLong(authentication.getName());
        return ResponseEntity.ok(areaTaskService.kpis(userId, areaId, priority));
    }

    @GetMapping("/{id:\\d+}")
    public ResponseEntity<AreaTaskResponseDto> getById(Authentication authentication, @PathVariable Long id) {
        Long userId = Long.parseLong(authentication.getName());
        return ResponseEntity.ok(areaTaskService.getById(userId, id));
    }
    ...
    public ResponseEntity<AreaTaskResponseDto> create(Authentication authentication,
        ...
        Long userId = Long.parseLong(authentication.getName());
        return ResponseEntity.status(HttpStatus.CREATED).body(areaTaskService.create(userId, dto));
    }

    @PutMapping("/{id:\\d+}")
    public ResponseEntity<AreaTaskResponseDto> update(Authentication authentication,
                                                      @PathVariable Long id,
                                                      @RequestBody UpdateAreaTaskDto dto) {
        Long userId = Long.parseLong(authentication.getName());
